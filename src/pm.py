...
def inputAndValidateMasterPassword():
    ...
    db = dbconfig()
    result = db[1].find_one()
    if hashed_mp != result['masterkey hash']:
        printc("[red][!] WRONG! [/red]")
        return None

    return [mp, result['device secret']]
...
def main():
    if args.option in ["add", "a"]:
        ...
        if res is not None:
            utils.add.addEntry(res[0], res[1], args.name,
                               args.url, args.email, args.login)
